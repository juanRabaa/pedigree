...
if( ! function_exists( 'wp_dropdown_posts' ) ) {
	...
	class RB_Inputs_Generator_Control extends RB_Extended_Control {
		...
		public function __construct($manager, $id, $args = array())
		{
			...
			$this->input_title = isset($args["inputs_title"]) ? $args["inputs_title"] : null;
		...
		public function print_inputs ( $inputs ){
			if ( !empty( $inputs ) ){
				foreach( $inputs as $input_id => $input_value ){
					...
					$reverse_dependencies = isset($input_data["reverse_dependencies"]) ? $input_data["reverse_dependencies"] : null;
					$this->print_single_input($input_id, $input_data["nice_name"], $input_data["type"], $dependencies, $reverse_dependencies, $input_value, $description);
				}
			}
		}
